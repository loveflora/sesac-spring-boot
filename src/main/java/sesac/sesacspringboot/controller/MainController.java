...
@Controller
// @RestController : Controller면서 모든 메소드가 @RequestBody를 갖도록 하는 친구
public class MainController {
    ...
    public String getIntroduce(@PathVariable(value = "name") String name,
                               Model model) {
        model.addAttribute("name", name);
        return "response";
    }
    ...
    public String getIntroduce(@RequestParam(value = "name") String name,
    ...
    @ResponseBody
            ...
            // 변수로 값을 하나씩 가져오는게 아니라, 객체에 값을 담아서 가져오고 싶을 때 사용하는 방법
    ...
    public String dtoResponse3(@RequestBody UserDTO userDTO) {
        // @RequestBody : 요청의 본문에 있는 데이터 (body)를 받아와서, 객체에 매핑 (필드 값에 값을 주입)
    ...
    public String axiosAPI3(@RequestParam(value="name", required = false) String name, @RequestParam(value="age", required = false) String age) {
        // 값이 왔어도 JSON 형태라면 읽지 못함
        // @RequestParam = @ModelAttribute = query string 으로 넘어온 데이터를 읽을 수 있다.
        // @RequestParam 에 required 가 true
        // required = false 라면, 필수 값이 아니기 때문에 error가 아닌, null 값으로 감
        String msg = "이름 : " + name + "\n나이 : " + age;
        return msg;
    }
    ...
    // Get 일반 : 가능
    // GET VO : null = @ModelAttribute (setter함수를 실행할 수 없어서)
    // Post 일반 : 실패
    // Post VO - RequestBody X: null = @ModelAttribute
    // Post VO - RequestBody O: 가능 => @RequestBody를 이용하면, setter 함수가 없어도 실행됨.
    ...
    @GetMapping("/vo/prac/response")
    public String getPracRes(){return "voPractice";}
    @PostMapping("/vo/prac/response")
    @ResponseBody
